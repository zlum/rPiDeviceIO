...
public:
    ...
    template<typename Type> Type read(const Register& reg);
    template<typename Type, typename Value> bool write(const Register& reg, const Value& val);
...
Type Hardware<Register, Byte>::read(const Register& reg)
{
...
bool Hardware<Register, Byte>::write(const Register& reg, const Value& val)
{
    std::memset(_io_buf, 0, IO_LEN_MAX);
    constexpr uint8_t size = sizeof(Type) > IO_LEN_MAX ?
                                 IO_LEN_MAX : uint8_t(sizeof(Type));
